...
\begin{align}
    \frac{\abs{\Omega_{\text{A} \leftrightarrow \text{E}}}^2}{\Delta^2} \Gamma t \ll 1.
\end{align}
Although this is the dominant loss in the system, the remaining fraction of the atom laser may not be fully transferred into the lasing condensate.  The transfer process must also be considered to determine the overall behaviour of the pumping mechanism in this limit.

In the limit of large detuning the suppression of spontaneous emission means that the photon and atom laser fluxes need not be exactly balanced for a steady-state to be reached, as in the resonant case.  To study the steady-state of the system in the limit that the source and lasing condensate numbers are not changing too quickly, the equations of motion may be simplified by moving into appropriate rotating frames to remove simple phase rotation and neglecting spontaneous emission.  In this limit the source and lasing condensates can be assumed to be constant in time, and the Rabi frequency $\Omega_{\text{A} \leftrightarrow \text{E}}$ will be sufficiently large to be negligibly reduced after propagation through the system and may be assumed to be constant in space.  Our interest is principally in the transfer process itself.  This may be investigated by considering the simplified propagation equations for the atom laser $\Psi_\text{A}$ and the $\Omega_{\text{L} \leftrightarrow \text{E}}$ optical mode as they traverse the lasing condensate in steady state,
\begin{subequations}
    ...
    \end{align}
\end{subequations}
where the `$\pm$' sign in \eqref{OpticalPumping:TravellingBeam:FarDetuned:SteadyStateEvolution:OmegaLE} corresponds to propagation of $\Omega_{\text{L} \leftrightarrow \text{E}}$ in the $\pm x$ direction.  These equations can be solved analytically as the phases of $\Psi_\text{L}$ and $\Omega_{\text{A} \leftrightarrow \text{E}}$ may be absorbed into the other terms without changing the form of the equations.

The solution to the system for the `$+$' sign of \eqref{OpticalPumping:TravellingBeam:FarDetuned:SteadyStateEvolution:OmegaLE} (the $0 \hbar k$ momentum-transfer process) may be found by applying the boundary conditions
\begin{subequations}
    \begin{align}
    ...
    \end{align}
\end{subequations}
where $x=0$ is chosen to be between the source and lasing condensates (see \figureref{OpticalPumping:TravellingBeamZeroDetuningResults}).  The steady-state solution for the atom laser can be shown to be
\begin{align}
...
\end{align}
As all atoms transferred out of the atom laser must go into the lasing condensate (as spontaneous emission has been neglected), the efficiency of the transfer process itself is given by
\begin{align}
...
\end{align}
where the integral has been extended to $+\infty$ as $\Psi_\text{L}(x\geq 0) = 0$ (refer to \figureref{OpticalPumping:TravellingBeamZeroDetuningResults}).  Efficient operation of this pumping process therefore requires
\begin{align}
    \frac{1}{4} \frac{\abs{\Omega_{\text{A} \leftrightarrow \text{E}}}}{\Delta} \sqrt{\frac{M \Gamma \sigma_0}{\hbar k A_\perp}} \int_{-\infty}^{+\infty} \abs{\Psi_\text{L}}\, dx \gg 1.
\end{align}

An illustration of the results of this process are in blah blah blah.

The $0\hbar k$ has a number of features that make it a promising scheme for an atom laser, blah blah blah.  But overlapping condensates are bad.  One way to avoid that would be...




A comparison between this equation and the other requirement yields the interesting conclusion that losses cannot simply be suppressed through appropriate choice of $\Omega_{\text{A} \leftrightarrow \text{E}}$ and detuning as both depend on their ratio.  Instead, the losses are reduced as the lasing condensate increases in size provided that $\abs{\Omega_{\text{A} \leftrightarrow \text{E}}} / \Delta$ is reduced accordingly.



The behaviour in the case of the `$+$' sign displays a number of useful properties, as $\Omega_{\text{A} \leftrightarrow \text{E}}$ is increased, good stuff happens, yada yada yada.


The system of equations \eqref{OpticalPumping:TravellingBeam:FarDetuned:SteadyStateEvolution} may be solved analytically in terms of the dimensionless quantity 
\begin{align}
    w(x) &= \frac{1}{4} \frac{\abs{\Omega_{\text{A} \leftrightarrow \text{E}}}}{\Delta} \sqrt{ \frac{M \Gamma \sigma_0}{\hbar k A_\perp}} \int_{-\infty}^x \abs{\Psi_\text{L}}\, dx.
\end{align}



The phases of $\Psi_\text{L}$ and $\Omega_{\text{A} \leftrightarrow \text{E}}$ may be absorbed into the other terms without changing the equations


This yields solutions with eigenvalues satisfying
\begin{align}
    \lambda^2 &= \pm \frac{M}{16 \hbar k} \frac{\Omega_{\text{A} \leftrightarrow \text{E}}^2}{\Delta^2} \Gamma \sigma_0
\end{align}

The co-propagating solution is
\begin{align}
    \Psi_\text{A}(+\infty) &= \Psi_\text{A}(0) \sech \left( \frac{1}{4} \frac{\abs{\Omega_{\text{A} \leftrightarrow \text{E}}}}{\Delta} \sqrt{\frac{M \Gamma \sigma_0}{\hbar k A_\perp}} \int_{-\infty}^{+\infty} \abs{\Psi_\text{L}}\, dx\right)
\end{align}
The co-propagating solution is approximately (in the limit of goodness),
\begin{align}
    \Psi_\text{A}(x) &= \Psi_\text{A}(0) \exp \left( \frac{1}{4} \frac{\abs{\Omega_{\text{A} \leftrightarrow \text{E}}}}{\Delta} \sqrt{\frac{M \Gamma \sigma_0}{\hbar k A_\perp}} \int_{-\infty}^x \abs{\Psi_\text{L}} \, dx\right)
\end{align}


The counter-propagating solution is
\begin{align}
    \Psi_\text{A}(x) &= \Psi_\text{A}(0) \cos \left(\frac{1}{4} \frac{\abs{\Omega_{\text{A} \leftrightarrow \text{E}}}}{\Delta} \sqrt{\frac{M \Gamma \sigma_0}{\hbar k A_\perp}} \int_{-\infty}^x \abs{\Psi_\text{L}} \, dx\right)
\end{align}
While the counter-propagating solution could certainly be made to work in 1D, it would be significantly harder in higher dimensions as the line integral of $\abs{\Psi_\text{L}}$ will vary across the condensate.
