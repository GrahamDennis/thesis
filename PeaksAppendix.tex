...
Finally, if the $\hat{Q}(\bm{k}, t)$ and $\hat{Q}'(\bm{k}, t)$ operators correspond to only two distinct eigenvalues (i.e. $\lambda = \lambda^*$), then by the uniqueness of the eigenvectors $\hat{\Lambda}_1'(\bm{k}, t) = \hat{\Lambda}_1(\bm{k}, t)$. For this case $\displaystyle e^{i n\omega T} = \pm 1$, making \eqref{PeaksAppendix:LambdaOperatorEvolution} and \eqref{PeaksAppendix:LambdaPrimeOperatorEvolution} consistent.
