...
\begin{align}
    \left.-\frac{d}{dt} N_\text{source}\middle/N_\text{source}\right. &\approx \frac{\abs{\Omega}^2}{\Delta^2} \left(\Gamma + \Gamma'\right),
\end{align}
...
\begin{figure}
    ...
    \caption{Level diagram for the continuous pumping experiment.  RF outcoupling couples the levels in the $F=2$ manifold (not pictured), while $\pi$-polarised optical pumping light resonantly drives $F=2 \leftrightarrow F'=1$ transitions.  Although this light couples to several levels, its purpose is to drive $\ket{F=2, m_F=0}$ atoms into the $\ket{F'=1, m_F=0}$ state from which they may be stimulated to decay into the lasing BEC in the $\ket{F=1, m_F=-1}$ state and emit $\sigma^+$-polarised light.}
    \label{OpticalPumping:LevelDiagram}
\end{figure}

Although our aim is to obtain a model with which the continuous pumping experiment described in \sectionref{OpticalPumping:ContinuousExperiment}, we also wish to be able to consider the limit in which the pumping light is significantly detuned (by several GHz).  In either case, the only relevant optical transitions are those in the $\text{D}_2$ line (those pictured in \figureref{OpticalPumping:LevelDiagram}) as all other levels are sufficiently detuned to be negligibly populated.  The transitions closest in frequency to the $\text{D}_2$ line are those of the $\text{D}_1$ line which are detuned by $7 \times \unit[10^{12}]{Hz}$, significantly greater than the maximum expected detuning from the $\text{D}_2$ line.
...
The first simplification we will make to \eqref{OpticalPumping:AtomLightHamiltonian} is to assume that all atomic and optical modes are coherent states, \emph{i.e.} the mean-field approximation.  While this approximation is justified for the condensed atomic fields and the strongly-occupied parts of the optical field (due to the pumping laser and stimulated emission into the lasing BEC), the vacuum fluctuations responsible for spontaneous emission are necessarily neglected by any mean-field model.  The effect of spontaneous emission may be partially included by adding a decay term for the excited atomic levels, however this necessarily neglects the effect of atoms that have undergone spontaneous emission.  While some of these atoms will decay into internal atomic states that are dark to the optical pumping laser and photons emitted by stimulated emission, others will decay into the same internal atomic level as the target condensate, however with non-zero momentum.  In this case, these atoms will be able to absorb photons emitted when atoms are stimulated to emit into the lasing condensate and the photon emitted when they decayed could also be absorbed by atoms in the lasing condensate.  Both of these processes are neglected by this treatment of spontaneous emission.  The effect of this approximation is discussed further in \sectionref{OpticalPumping:Discussion}\footnote{FIXME: Cross-ref to unwritten content. Check at a later date.}.
...
Our next approximation is to neglect the fast-rotating parts of the atom--light coupling terms in \eqref{OpticalPumping:HeisenbergEvolution}, \emph{i.e.} the rotating wave approximation (RWA).  We achieve this by first separating the purely real electric field into positive and negative frequency terms
\begin{align}
...
Using \eqref{OpticalPumping:ElectricFieldPhotonFieldRelationship} the photon field may be eliminated in favour of the electric field giving the equation of motion for the electric field to be
\begin{align}
...
\end{align}
This equation has the familiar form of a transport process in the $\unitvec{q}_n$ direction (at speed $c$) with a source term. The propagation of the electric field across the system occurs on a sufficiently short timescale that as seen by the atoms, the velocity is essentially infinite.  By separating the fast phase rotation of the electric and atomic fields from the slowly-varying envelopes, it will become possible to neglect the finite propagation time of the electric field in \eqref{OpticalPumping:ElectricFieldEvolution} to obtain a set of algebraic equations for the electric field at a given time.  The next step in this process is to remove the fast phase rotation of these fields.
...
\begin{subequations}
    ...
    \end{align}
\end{subequations}
where $\omega_i$ (and $\omega_{i'}$) are the bare frequencies of the atomic levels, $\Delta_i$ (and $\Delta_{i'}$) are the detunings of those levels.  The definitions of these detunings are illustrated in \figureref{OpticalPumping:PhaseRotationDefinition}.
...
The equations of motion of the slowly-varying fields are
\begin{subequations}
    \begin{align}
        \begin{split}
        ...
        \begin{split}
          ...
          &\relphantom{=} + i \frac{\hbar \abs{\vect{q}_n}}{2 \varepsilon_0} \unitvec{u}^*_{\lambda_n}(\vect{q}_n) \cdot \sum_{\{i,j'\}\in T_n} \frac{\vect{d}_{ij'}}{\hbar} \widetilde{\Psi}^*_i(\vect{x}) \widetilde{\Psi}_{j'}(\vect{x}) e^{-i \left[\omega_{j'} - \Delta_{j'} -(\omega_i - \Delta_i) - \omega_{\vect{q}_n}\right] t}.
        \end{split}
    \end{align}
\end{subequations}

FIXME: We need a substantial explanation here.  There are a bunch of requirements, if the state is initially occupied, the detuning must be zero (actually only a requirement when we make the adiabatic elimination, but might as well discuss it here); the photon energies must be consistent, \emph{i.e.} a given atomic state may only be occupied at a given energy. 
All of the exponential terms are $1$ in our case as all of the photon energies are consistent, \emph{i.e.} $\omega_{j'} - \Delta_{j'} - (\omega_i - \Delta_i) = \omega_{\vect{q}_n} \forall \; i, j', n : \{i, j'\} \in T_n$. See \figureref{OpticalPumping:LevelDiagram}.
...
\end{align}
But we only make this transform on the electric fields, not the atomic fields.  This means that
\begin{align}
...
\begin{align}
  \begin{split}
    ...
    &\relphantom{=} + i \frac{\hbar \abs{\vect{q}_n}}{2 \varepsilon_0} \unitvec{u}^*_{\lambda_n}(\vect{q}_n) \cdot \sum_{\{i,j'\}\in T_n} \frac{\vect{d}_{ij'}}{\hbar} \widetilde{\Psi}^*_i\left(\vect{x}', t(\vect{x}', t')\right) \widetilde{\Psi}_{j'}\left(\vect{x}', t(\vect{x}', t')\right)
  \end{split}
\end{align}

If we define our spatial origin to be in the centre of the condensate, then $t(\vect{x}', t') \approx t'$ is a very good approximation.  To see just how good, consider that
\begin{align}
...
This correction term is of the order of 
\begin{align}
...
\end{align}
Although this is a zeroth-order approximation, it is as good as (or better than) many other first-order approximations that have been made earlier in the derivation.  A similar approximation needs to be made for $\widetilde{E}_{+, \vect{q}_n}(\vect{x}, t) \approx \widetilde{E}_{+, \vect{q}_n}(\vect{x}, t')$ with a similarly-sized correction.  These approximations made, the system becomes
\begin{subequations}
    \begin{align}
        \begin{split}
            i \hbar \frac{\partial}{\partial t} \widetilde{\Psi}_i(\vect{x}) &= \left( -\frac{\hbar^2 \nabla^2}{2M} + V_i(\vect{x}) + U \sum_j \abs{\widetilde{\Psi}_j(\vect{x})}^2 - \hbar \omega_{i} + \hbar \Delta_i \right) \widetilde{\Psi}_i(\vect{x})\\
        ...
        \end{split}\\
      \unitvec{q}_n \cdot \nabla \widetilde{E}_{+, \vect{q}_n}(\vect{x}) &= i \abs{\vect{q}_n} \widetilde{E}_{+, \vect{q}_n}(\vect{x}) + i \frac{\hbar \abs{\vect{q}_n}}{2 \varepsilon_0} \unitvec{u}^*_{\lambda_n}(\vect{q}_n) \cdot \sum_{\{i,j'\}\in T_n} \frac{\vect{d}_{ij'}}{\hbar} \widetilde{\Psi}^*_i\left(\vect{x}\right) \widetilde{\Psi}_{j'}\left(\vect{x}\right)
    \end{align}
\end{subequations}

FIXME: Here we need a computational aside where we discuss how this system can be solved using cross-propagation, and how particular care must be taken in cases in which the optical fields do not all propagate in the same direction.  In this case the system of equations must be solved self-consistently.  The system has a few properties which help in this (namely slowly-varying) which enables self-consistent solutions to be found iteratively.

At this point we may adiabatically eliminate the $\widetilde{\Psi}_{i'}(\vect{x})$ atomic excited states.  We do this by recognising that the $\hbar \omega_{i'}$ term cancels with the other energy terms in parenthesis giving
\begin{align}
...
Now we just need to summarise the approximations made in the derivation, and maybe draw a diagram or something.


If everything is hunky-dory, then we can approximate $t(\vect{x}', t') \approx t'$, similarly elsewhere, then we make adiabatically eliminate the excited states.  I fear that there is a self-consistency requirement in here that we are approximating away.  I fear that despite the terms involved being `small', they can have a significant effect due to the way in which they appear.  Nevertheless, approximate we must.
...
\begin{align}
    \hat{\vect{E}}(\vect{x}) &= \sum_\lambda \frac{1}{(2\pi)^{3/2}} \int d \vect{k}\, \left[\unitvec{u}_\lambda(\vect{k}) \left(\frac{\hbar \omega_{\vect{k}}}{2\varepsilon_0} \right)^{\frac{1}{2}} \hat{\Phi}_\lambda(\vect{k}) e^{i \vect{k} \cdot \vect{x}} + \text{h.c.}\right],
\end{align}
where $\unitvec{u}_\lambda(\vect{k})$ is a unit vector for the corresponding polarisation, $\hat{\Phi}_\lambda(\vect{k})$ is the photon annihilation operator in momentum space for the $\lambda$ polarisation, $\lambda = \pm 1$.
...
\begin{align}
    \hat{\vect{E}}(\vect{x}) &\approx \sum_\lambda \frac{1}{(2\pi)^{3/2}} \left(\frac{\hbar k_\gamma c}{2\varepsilon_0} \right)^{\frac{1}{2}} \int d \vect{k}\, \left[\unitvec{u}_\lambda \hat{\Phi}_\lambda(\vect{k}) e^{i \vect{k} \cdot \vect{x}} + \text{h.c.}\right],
\end{align}
