...
\begin{figure}
    \centering
    \includegraphics[width=8cm]{LambdaModel}
    \caption{Lambda model. FIXME: This is not a caption}
    \label{OpticalPumping:LambdaModel}
\end{figure}
...
For the 1D scheme presented in \figureref{OpticalPumping:LambdaModel},
\begin{subequations}
    \begin{align}
        ...
        \frac{d}{dx} \Omega_1 &= i k \Omega_1 + \frac{i}{4} f_1 \Gamma \sigma_0 \frac{\Psi_1^* \Psi_2^{\phantom{*}}\Omega_2}{\Delta - \frac{i}{2} \Gamma}\\
        \pm \frac{d}{dx} \Omega_2 &= i k \Omega_2 + \frac{i}{4} f_2 \Gamma \sigma_0 \frac{\Psi_2^* \Psi_1^{\phantom{*}} \Omega_1}{\Delta - \frac{i}{2} \Gamma}
    \end{align}
\end{subequations}
where $\hbar \Omega_n = \vect{d}_{i j'} \cdot \unitvec{u}_{\lambda_n} \widetilde{E}_{+, \vect{q}_n}$, $\displaystyle \Gamma = \frac{\omega^3 d_\text{eff}^2}{3 \pi \epsilon_0 \hbar c^3}$, $\abs{\vect{d}_{ij'}} = f_{ij'} \abs{d_\text{eff}}^2$, $\sigma_0 = \frac{3 \lambda^2}{2 \pi}$, stuff.
