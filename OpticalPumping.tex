...
\end{align}
The net rate of atom transfer into the lasing mode ($\ket{2}$) is therefore
\begin{align}
    \frac{d N_2}{dt} &= \abs{\Omega_1(-\infty)}^2 \frac{8 A_\perp}{\Gamma \sigma_0} \frac{N_1}{N} \frac{N_2}{N} \abs{\alpha - 1}^2,
\end{align}
where
\begin{align}
...
\end{align}
$N_i$ is the number of atoms in the $\ket{i}$ state, and $N$ is the number of atoms in the system.  The efficiency of the atomic transfer is
\begin{align}
...
In the limit of zero detuning ($\Delta = 0$) and assuming the lasing mode to contain significantly more atoms than the source mode ($N_1 \ll N_2$), a simpler understanding of the dynamics can be obtained,
\begin{align}
    \alpha &= \exp\left( - \frac{N \sigma_0}{4 A_\perp}\right) \approx 0, \\
    \frac{d N_2}{dt} &\approx \abs{\Omega_1(-\infty)}^2 \frac{8 A_\perp}{\Gamma \sigma_0} \frac{N_1}{N_2}, \\
    \eta &\approx \frac{1}{2}.
\end{align}
Where for $\lambda = \unit[780]{nm}$, $N= 5 \times 10^5$, $A_\perp = \unit[10^{-9}]{m\textsuperscript{2}}$, the term in brackets is $\sim -36$. Here we observe the Quantum Zeno Effect, \emph{i.e.} the opposite of Bose-enhancement.  If there are more atoms in the lasing mode, the rate of transfer to that mode \emph{decreases}.  Simulations are in agreement with this result, and show that the efficiency increases from 50\% for later times as the atoms are closer to being in a dark state\dots

% \begin{align}
%     \frac{d N_2}{dt} &= \abs{\Omega_1(-\infty)}^2 \frac{8 A_\perp}{\Gamma \sigma_0} \frac{N_1}{N} \frac{N_2}{N} \abs{\exp \left( i \frac{N \Gamma \sigma_0}{8 A_\perp(\Delta - \frac{i}{2} \Gamma)}\right) - 1}^2
% \end{align}
% 
% \begin{align}
%     \Omega_1(x) &= \Omega_1(-\infty) \left\{1 + \frac{\abs{\Psi_1}^2}{\abs{\Psi_1}^2 + \abs{\Psi_2}^2}\left[\exp\left(i \frac{\Gamma \sigma_0}{8 A_\perp (\Delta - \frac{i}{2} \Gamma)} \int_{-\infty}^x dx\, \abs{\Psi_1}^2 + \abs{\Psi_2}^2 \right) -1 \right] \right\}
% \end{align}
% 
% \begin{align}
%     \abs{\Omega_1(+\infty)}^2 &= \abs{\Omega_1(-\infty)}^2 \abs{1 + \frac{N_1}{N} \left[\exp\left( i \frac{N \Gamma \sigma_0}{8 A_\perp(\Delta - \frac{i}{2}\Gamma)}\right) - 1\right] }^2
% \end{align}
