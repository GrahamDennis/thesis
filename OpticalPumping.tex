...
\begin{figure}
    ...
    \caption{Overlapping condensate model. When the `$+$' sign is chosen in \eqref{OpticalPumping:OverlappingCondensates:Omega2}, the optical field $\Omega_2$ propagates from the left [marked with $(+)$], while if the `$-$' sign is chosen it will propagate from the right [marked with $(-)$]. FIXME: This is still not a caption.}
...
\begin{subequations}
    \begin{align}
        ...
        \frac{d}{dx} \Omega_1 &= i k \Omega_1 + \frac{i}{4} f_{13} \Gamma \sigma_0 \frac{\abs{\Psi_1}^2 \Omega_1 + \Psi_1^* \Psi_2^{\phantom{*}}\Omega_2}{\Delta - \frac{i}{2} \Gamma}\\
        \pm \frac{d}{dx} \Omega_2 &= i k \Omega_2 + \frac{i}{4} f_{23} \Gamma \sigma_0 \frac{\abs{\Psi_2}^2 \Omega_2 + \Psi_2^* \Psi_1^{\phantom{*}} \Omega_1}{\Delta - \frac{i}{2} \Gamma} \label{OpticalPumping:OverlappingCondensates:Omega2}
    \end{align}
\end{subequations}
where $\hbar \Omega_i = \vect{d}_{i 3} \cdot \unitvec{u}_{\lambda_i} \widetilde{E}_{+, \vect{q}_i}$, $\displaystyle \Gamma = \frac{\omega^3 d_\text{total}^2}{3 \pi \epsilon_0 \hbar c^3}$, $\abs{d_\text{total}}^2 = \sum_{ij'} \abs{\vect{d}_{ij'}}$, $\abs{\vect{d}_{ij'}}^2 = f_{ij'} \abs{d_\text{total}}^2$, $\displaystyle\sigma_0 = \frac{3 \lambda^2}{2 \pi}$, $f_{ij'}$ is the fraction of atoms in the $j'$ state that spontaneously decay to the $i$ state.  If the state $\ket{3}$ may only decay into states $\ket{1}$ and $\ket{2}$ and does so with equal probability, then $f_{13} = f_{23} = \frac{1}{2}$.
