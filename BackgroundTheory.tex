...
In this section the appropriate master equation terms for modelling Penning ionisation are obtained. In doing the ionisation of the $S=2$ quasimolecule states is neglected as the rate constant is 5 orders of magnitude smaller than that for the $S=0$ quasimolecule states at BEC temperatures.

Expressed in terms of the atomic states $\hat{\Psi}_i$ in the $F=1$ total angular momentum manifold where $i$ is the angular momentum project, the quasimolecule state that undergoes Penning ionisation is given by
\begin{align}
...
From the master equation \eqref{BackgroundTheory:PenningIonisationMasterEquationTermArbitraryConstant}, the equation of motion for the expectation value of the total number of atoms can be obtained
\begin{align}
    ...
    \end{split}
\end{align}
For a thermal state, the last pair of terms in \eqref{BackgroundTheory:NEquationOfMotion} are zero, and $\mean{\hat{a}^\dagger \hat{a}^\dagger \hat{a} \hat{a}} = 2 \mean{\hat{n}}^2$. In terms of the number densities in each state $n_j(\bm{x})$, the equation of motion for the total number of atoms in a thermal sample is
\begin{align}
