...
As discussed in \sectionref{Introduction:AtomLaser}\footnote{FIXME: We can expect that the idea of an atom laser has been discussed in the introduction.  Check cross-ref at later date.} an atom laser is formed by outcoupling from a condensate to produce a propagating beam of coherent atoms.  In the common case of a magnetically-trapped condensate in an $F=1$ manifold, the atoms in one of the $m_F$ Zeeman levels are trapped (the low-field seeking level, either $m_F=1$ as in He* or $m_F=-1$ as in \nucl{87}{Rb}), the atoms in the $m_F=0$ level experience no magnetic trapping potential (and are referred to as `untrapped'), and the atoms in the level opposite to the trapped state experience a repulsive potential and are referred to as `anti-trapped'.  For He*, the potentials experienced by the three levels in a cylindrically-symmetric trap are
\begin{align}
    V_{m_F}(\vect{x}) &= m_F V_\text{trap}(\vect{x}) = m_F \frac{1}{2} M \left(\omega_r^2 x^2 + \omega_r^2 y^2 + \omega_z^2 z^2 \right).
\end{align}
These three levels are coupled by applying radio-frequency radiation resonant with the Zeeman splitting between the $m_F$ levels.  This Zeeman splitting is due to the magnetic trap, and therefore the outcoupling process will be resonant along a surface of constant $V_\text{trap}(\vect{x})$.  The Hamiltonian for this coupling term is
\begin{align}
    \hat{H}_\text{outcoupling} &= \sum_{m_F} \int d \vect{x}\, \hbar \Omega \left(\hat{\Psi}_{m_F+1}^\dagger(\vect{x}) \hat{\Psi}_{m_F}^{\phantom{\dagger}}(\vect{x}) + \hat{\Psi}_{m_F}^\dagger(\vect{x}) \hat{\Psi}_{m_F+1}^{\phantom{\dagger}}(\vect{x}) \right),
\end{align}
where $\Omega$ is the Rabi frequency.  Note that this Hamiltonian only applies for an $F=1$ manifold.  

\begin{figure}
    \centering
    \includegraphics[width=15cm]{OutcouplingSurfaces}
    \caption{FIXME: This is not a caption.}
    \label{BackgroundTheory:OutcouplingSurfaces}
\end{figure}


The equations of motion for the system are therefore
\begin{subequations}
    \begin{align}
        \begin{split}
...
Initially the condensate is in the ground state of the trapped $m_F=1$ state.  Once the outcoupling is turned on, the states will be coupled and atoms will be transferred to the $m_F=0$ and $m_F=-1$ levels.  In the limit of weak outcoupling, the atoms in the $m_F=0$ level will leave the region in which outcoupling is resonant before they are significantly coupled into the $m_F=-1$ level.  The $m_F=-1$ level will therefore be relatively unpopulated and may be neglected.  It is the untrapped $m_F=0$ level that we are primarily interested in as it is in this level that the freely-falling atom laser forms.

For short times, most of the atoms will be in the $m_F=1$ level and we may use the Thomas-Fermi approximation to determine the effective potential seen by the untrapped $m_F=0$ atoms.  Inside the condensate, the effective potential has contributions from gravity and interatomic interactions,
\begin{align}
    V_\text{eff} &= M g y + U_\text{int} \abs{\Psi_1}^2 = M g y + \left(\mu - V_\text{trap}(\vect{x}) - M g y \right) \notag\\
...
Due to gravity, the centre of the condensate and the centre of the magnetic trap do not coincide.  The atoms are pulled down by gravity and the centre of the condensate is $\Delta y = g / \omega_y^2$ below the centre of the condensate.  

Quote: ``However, as discussed in \sectionref{BackgroundTheory:TransverseProfile} the evolution of the atom laser in the region below the BEC is well described by free fall with the exception of fine-scale interference effects.''
``The double-peak structure of the atom laser profile was discussed earlier in \sectionref{BackgroundTheory:TransverseProfile}.''
``Neither the fine structure nor the `shadow' of the BEC in the atom laser profile discussed in \sectionref{BackgroundTheory:TransverseProfile} are observed\dots''
