...
The purpose of this chapter has been to investigate the feasibility of producing a continuously pumped atom laser driven by evaporation and collisions with a cloud of thermal atoms.  The method here has been to investigate the best-case scenario in which the replenishment process introduces no additional heating to the trapped thermal component beyond that due to bringing the replenishing atoms into contact with the thermal cloud.  With these caveats in mind, the results of this chapter are promising: using an existing experimental source \citep{Muller:2007} it appears possible to produce steady-state condensates with large atom number ($\sim 5\times 10^5$ atoms) using the scheme presented in \sectionref{KineticTheory:Scheme}.  Could the atomic flux of this source be increased by an order of magnitude, the condensate number produced by this scheme could be pushed to $5\times 10^6$ atoms.  

Ultimately it is not the size of the condensate that we are interested in, but the atom flux of the atom laser produced and its coherence length.  The former goal will certainly be improved by larger equilibrium condensate sizes, however the atom laser itself will be useless unless its coherence length is sufficiently larger than that of cold thermal sources to offset its necessarily reduced atom flux.  It is by no means clear from the present work whether the atom laser produced from the atomic source of \citet{Muller:2007} will have a sufficiently large coherence length to be useful.  To investigate this point it will be necessary to not make the assumption of ergodicity and to include the full spatial dependence of the Raman outcoupler.  This can be achieved through using a more general kinetic theory such as the so-called `ZNG theory' \citep{Zaremba:1999,Proukakis:2008} or the Stochastic Projected Gross-Pitaevskii equation (SPGPE) \citep{Blakie:2008a}.
