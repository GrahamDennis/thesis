...
The dipole moments of the various transitions can be calculated as multiples of the reduced dipole matrix element for the $\text{D}_2$ transition by the Wigner-Eckart theorem \citep{Eckart:1930,Brink:1962}
\begin{align}
    \vect{d}_{F,f;F',f'} &= c_{F,f;F',f'} d_\text{reduced} \unitvec{u}_\alpha,
\end{align}
where $\alpha\in \{\pi,\sigma^\pm\}$ is the polarisation corresponding to the $\ket{F, f} \leftrightarrow \ket{F', f'}$ transition, $d_\text{reduced} = 3.6 \times \unit[10^{-29}]{C.m}$ is the reduced dipole matrix element for the $\text{D}_2$ transition in $^{87}\text{Rb}$, and $c_{F, f; F', f'}$ is a real constant that may be found in standard tables\footnote{FIXME: Cite something.}.  For example the two most important dipole moments are
\begin{align}
...
1D dimension reduction, then move on to a discussion of results.
