...
We should now be able to do an inverse Fourier transform. We can once we approximate $\abs{\vect{k}} \approx \frac{\vect{q}_n}{\abs{\vect{q}_n}} \cdot \vect{k}$,
\begin{align}
    ...
    &\approx \abs{\vect{q}_n} + \frac{\vect{q}_n \cdot \delta \vect{k}}{\abs{\vect{q}_n}} = \abs{\vect{q}_n} + \frac{\vect{q}_n \cdot \left(\vect{k} - \vect{q}_n\right)}{\abs{\vect{q}_n}}\\
    &= \frac{\vect{q}_n}{\abs{\vect{q}_n}} \cdot \vect{k}
\end{align}
 Then we obtain
\begin{align}
    i \hbar \frac{\partial}{\partial t} \Phi_{\lambda_n, \vect{q}_n}(\vect{x}) &= -i \hbar c \frac{\vect{q}_n}{\abs{\vect{q}_n}} \cdot \nabla \Phi_{\lambda_n, \vect{q}_n}(\vect{x}) - \left(\frac{\hbar \omega_{\vect{q}_n}}{2\varepsilon_0}\right)^{\frac{1}{2}} \vect{u}_{\lambda_n}^*(\vect{q}_n) \cdot \sum_{\{i,j'\} \in T_n} \vect{d}_{ij'} \Psi^*_i(\vect{x}) \Psi_{j'}(\vect{x})
\end{align}
...
\begin{align}
  \vect{x}' &= \vect{x} \\
  t' &= t - \frac{1}{c} \frac{\vect{q}_n \cdot \vect{x}}{\abs{\vect{q}_n}} 
\end{align}
This means that
\begin{align}
  \frac{\partial}{\partial t} &= \frac{\partial }{\partial t'}\\
  \nabla_{\vect{x}} &= \left(\nabla_{\vect{x}} t'\right) \frac{\partial }{\partial t'} + \nabla_{\vect{x}'}\\
  &= - \frac{1}{c} \frac{\vect{q}_n}{\abs{\vect{q}_n}}\frac{\partial}{\partial t'} + \nabla_{\vect{x}'}
\end{align}
This gives our equation for $\widetilde{E}_{+, \vect{q}_n}$ as
\begin{align}
  \begin{split}
    \frac{1}{c} \frac{\partial}{\partial t'} \widetilde{E}_{+, \vect{q}_n} (\vect{x}', t') &= - \frac{\vect{q}_n}{\abs{\vect{q}_n}} \cdot \left(- \frac{1}{c} \frac{\vect{q}_n}{ \abs{\vect{q}_n}} \frac{\partial}{\partial t'} + \nabla_{\vect{x}'} \right) \widetilde{E}_{+, \vect{q}_n}(\vect{x}', t') + i \abs{\vect{q}_n} \widetilde{E}_{+, \vect{q}_n}(\vect{x}', t')\\
