...
Measurement is an essential part of our developed world.  We measure the quantities of ingredients when following a recipe, the sizes of pieces of wood when making a cabinet, and we obsessively measure time for fear of running out of it.  Many of these measurements are quite imprecise: ``The recipe only asks for $\unit[100]{g}$ of chocolate, but I'll put the rest in anyway.'', or the notorious ``I'll be with you in a minute\dots''.  Very precise measurements are, however, making an increasing impact on our lives, although usually without our realising it.  Perhaps the best example of this is the Global Positioning System (GPS), which uses a system of satellites orbiting the Earth, each of which containing an atomic clock that enables the very precise measurement of time.  By comparing the arrival times of the precisely synchronised signals of these satellites, it is possible to determine one's position on Earth to an accuracy of better than $\unit[1]{m}$, even without a precision clock at the receiver.

One way of performing precise measurements is to compare the quantity to be measured against a known scale and count the number of divisions.  For example, when measuring the length of a piece of string with a ruler\footnote{FIXME: I need more interesting example of something to measure with a ruler.}, one counts the number of divisions between the ends of the string.  Fortunately, many of the divisions are typically numbered, making this an easier task.  The precision of this kind of measurement is limited by the smallest division on the ruler.  If I want to know how long my piece of string is to the nearest micron, I will need a measurement device with a smaller division.

An alternative way of performing precise measurements is with interferometry.  At its heart, interferometry is about the measurement of the difference in the readings of two oscillators.  This difference can be measured to much less than a single division (a cycle of the oscillator).  The precision with which this difference 


The precision with which this difference can be measured is limited by the standard quantum limit

The limit to which 

In this sense, it enables the 

someone has numbered many of the divisions 

count the number of `ticks'


One way of performing precise measurements is to count the number of `ticks' between two 


Interferometry is one way of performing precision measurements.  At its heart, interferometry is about the measurement of the difference in the readings of two clocks.  The advantage of this technique over its alternative, counting the number of ticks between two events, 

(counting the number of `ticks' )


Keeping with the clock analogy, the alternative method of making a measurement is to take

At its heart, interferometry is about the precise measurement of the differences

careful comparison of different clocks


There are two ways of performing measurements.  The first is to count the number of marks between two events


Interferometry is one way of performing precision measurements.  At its heart, interferometry is about the careful comparison of different clocks

Interferometry is one way of performing precise measurements.  


At its heart, interferometry is about the careful comparison of different clocks: frequently these clocks are the oscillations of photons, however it is also possible to perform interferometry with electrons \citep{Missiroli:1981}, neutrons \citep{Rauch:1974}, or atoms \citep{Keith:1991,Carnal:1991,Riehle:1991,Kasevich:1991}.  


Interferometry is not the only way of performing precise measurements, but it has the advantage that 




Clocks and oscillations are very simple systems, permitting high accuracy.  Stuff and nonsense.

By comparing clocks,

The advantage of interferometry is that stuff and nonsense.  


Interferometry is a nice way of performing...
...
Control: manipulation, we can have them insensitive to all kinds of things, but we can still manipulate them.  They're kind of cool. (cough).




Squeezing?  We can do that too!  Flux has always been the big winner, but stuff and nonsense.

Interferometry is a nice way of performing precision measurements because stuff and nonsense.
