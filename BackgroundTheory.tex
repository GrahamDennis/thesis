...
In the case of the anharmonic oscillator Hamiltonian \eqref{BackgroundTheory:AnharmonicOscillator:Hamiltonian}, the evolution equation for the Wigner distribution is
\begin{align}
...
\end{align}
The exact and truncated solutions are illustrated in \figureref{BackgroundTheory:TwoTimeTWComparison}.
...
The good agreement between the Truncated Wigner and exact results in \figureref{BackgroundTheory:TwoTimeTWComparison} for $\kappa t \lesssim \frac{\pi}{4}$ does not mean that the distributions themselves are approximately equal over this time.  The Wigner and Truncated Wigner distributions for the anharmonic oscillator are illustrated in \figureref{BackgroundTheory:WignerTruncatedWignerComparison}.  Expectation values of higher-order moments therefore diverge from the exact solutions faster than those for lower-order moments.  The validity of Truncated Wigner therefore also depends on the moments that are to be calculated.  Typically, it is only second order moments that are of interest $\mean{\hat{a}^\dagger \hat{a}}$, and this is the case in this thesis.
...
\end{align}
The equation of motion for $\Psi$ and $\delta\hat{\Psi}$ may be obtained from \eqref{BackgroundTheory:ExpectationValueEvolution} and \eqref{BackgroundTheory:LocalOperatorEvolution} respectively \citep{Proukakis:2008},
\begin{align}
    \begin{split}
...
The Bogoliubov-type methods rely on an explicit parametrisation of the state of the system.  There are a hierarchy of such methods.  These are summarised quite nicely in \citep{Proukakis:2008}.
